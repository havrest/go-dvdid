...
func ComputeDVDId(discPath string) ([]byte, error) {
    ...
    if os.IsNotExist(err) || !fileInfos.IsDir() {
        return nil, fmt.Errorf("\"VIDEO_TS\" could not be found in \"%s\" or is not a directory.", discPath)
    }
    ...
    if err != nil {
        return nil, fmt.Errorf("Failed to open directory \"%s\" to list files. %e", filepath.Join(discPath, "VIDEO_TS"), err)
    }
    ...
    if err != nil {
        return nil, fmt.Errorf("Failed to list files in directory \"%s\". %e", filepath.Join(discPath, "VIDEO_TS"), err)
    }
    ...
    for _, fName := range []string{"VIDEO_TS.IFO", "VTS_01_0.IFO"} {
        ...
        if os.IsNotExist(err) {
            return nil, fmt.Errorf("\"%s\" could not be found in \"%s\" or is not a directory.", filepath.Join("VIDEO_TS", fName), discPath)
        }
        ...
        if err != nil {
            return nil, fmt.Errorf("\"%s\" could not be opened in \"%s\" or is not a directory.", filepath.Join("VIDEO_TS", fName), discPath)
        }
        defer file.Close()
        ...
        if err != nil || count != int(readSize) {
        	return nil, fmt.Errorf("\"%s\" could not be read in \"%s\" or is not a directory.", filepath.Join("VIDEO_TS", fName), discPath)
        }
    ...
    return discIdCrc64.Sum(nil), nil
}
